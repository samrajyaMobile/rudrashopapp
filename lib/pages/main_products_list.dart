  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      Provider.of<MainProductsListModel>(context, listen: false)
          .getProductsList(context, widget.sub3CategoryId ?? "");
    });
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        body: Consumer<MainProductsListModel>(
          builder: (context, mainProducts, _) {
            return WillPopScope(
              onWillPop: () async {
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, int index) {
                        MainProductData? products =
                            mainProducts.productsList[index];
                        return InkWell(
                          onTap: () {
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => ProductsDetails(
                                          ...
                                          sub3CategoryId:
                                              products.sub3CategoryId,
                                          sub3CategoryName:
                                              products.sub3CategoryName,
                                          productName: products.productName,
                                          productSp:
                                              products.productSp.toString(),
                                          pGst: products.pGst.toString(),
                                          discount:
                                              products.discount.toString(),
                                          productMoq:
                                              products.productMoq.toString(),
                                          productsDic:
                                              products.productsDescr.toString(),
...
class MainProductsListModel extends ChangeNotifier {
  ...
  getProductsList(BuildContext context, String sub3Category) async {
    showDialog(
        context: context,
    ...
    var url =
        "${AppConstant.GET_MAIN_PRODUCTS}${QueryParamsConstant.SUB_CATEGORY_ID}=0&sub3CategoryId=$sub3Category";
